        ...
        <div class="flex justify-between">
            ...
            <div class="flex items-center">
                ...
                <button data-dropdown class="flex items-center px-3 py-2 focus:outline-none hover:bg-gray-200 hover:rounded-md" type="button" x-data="{ open: false }" @click="open = true" :class="{ 'bg-gray-200 rounded-md': open }">
                    <img src="https://i.ibb.co.com/yNDLTcr/admin.jpg" alt="Profle" class="h-8 w-8 rounded-full">
            ...
            <div>
                <div class="flex-1 p-6">
                    <header class="flex justify-between items-center bg-white p-4 rounded shadow mb-6">
                        ...
                        <form action="{{ route('penduduk.index') }}" method="GET" class="mb-5">
                            <input type="text" name="search" placeholder="Cari penduduk..." class="border rounded px-4 py-2" value="{{ request('search') }}">
                            <button type="submit" class="bg-blue-700 text-white font-bold py-2 px-4 rounded ml-2">Cari</button>
                        </form>
                    </header>
                    ...
                    <div class="bg-white p-6 rounded shadow">
                        <table class="table-auto border-collapse w-full">
                            ...
                            <tbody>
                                @forelse ($penduduk as $orang)
                                <tr>
                                    <td class="border px-4 py-2">{{ $loop->iteration }}</td>
                                    <td class="border px-4 py-2">{{ $orang->nik }}</td>
                                    <td class="border px-4 py-2">{{ $orang->nama }}</td>
                                    <td class="border px-4 py-2">{{ $orang->jenis_kelamin }}</td>
                                    <td class="border px-4 py-2">{{ $orang->alamat }}</td>
                                    <td class="border px-4 py-2">{{ $orang->pekerjaan }}</td>
                                    <td class="border px-4 py-2 flex justify-around">
                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('penduduk.destroy', $orang->id) }}" method="POST">
                                            <a href="{{ route('penduduk.show', $orang->id) }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded inline-block mb-1">DETAIL</a>
                                            <a href="{{ route('penduduk.edit', $orang->id) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded inline-block mb-1">EDIT</a>
                                            @csrf
                                            @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="13" class="border px-4 py-2 text-center">Data Penduduk Tidak Ada</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
